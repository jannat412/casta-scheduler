# casta-scheduler ![npm](https://img.shields.io/npm/v/casta-scheduler.svg?style=plastic)

A scheduler and resource planning component built for React and made for modern browsers (IE10+), IE needs babel-polyfill.  
**From the npm version 1.0.0, Scheduler will use responsive layout by default(set SchedulerData.config.schedulerWidth to a percentage instead of a number).**

## Version selection

- casta-scheduler@1.0.3

## Use and Setup

`npm install casta-scheduler --save`

```js
//1. import
import Scheduler, {SchedulerData, ViewTypes, DATE_FORMAT} from 'casta-scheduler'
//include `casta-scheduler/lib/css/style.css` for styles, link it in html or import it here
import 'casta-scheduler/lib/css/style.css'
...
- `isEventPerspective` is a bool value, if true, Scheduler will display the task view of current view type. In
  ...
  every slot describes how many events a check task is divided into and who will make it done. Add a `groupId` and
  `groupName` property to every event object, so that the events having the same `groupId` will belong to the same check task and
  ...
  [DemoData.js](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/DemoData.js) for details.
- `newConfig` is a config object, used to override the [default config](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/config.js)
  fully or partly.
- `newBehaviors` is a config object, used to override the [default behaviors](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/behaviors.js)
  fully or partly.
- `localeMoment` is a locale moment object, which is unified used in casta-scheduler. If not provided, Scheduler will come
...
See the demo `resources` in the [DemoData.js](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/DemoData.js).
...
See the demo `events` in the [DemoData.js](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/DemoData.js).
If we use the task view, we'd better add the `groupId` and the `groupName` property to each event object, see the
`eventsForTaskView` in the [DemoData.js](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/DemoData.js) for details.
...
Used to resolve the locale string of date label of Scheduler component.(Refer to the [getDateLabel](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/behaviors.js) func for example)

### 3.SchedulerData.config(See the [config.js](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/config.js) for details.)
...
The max height of Scheduler. If the desired height is checkger than the max height, the header row of Scheduler will be
...
Refer the `isNonWorkingTime` func in the [behaviors.js](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/behaviors.js).
...
Refer the `getSummary` func in the [behaviors.js](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/behaviors.js).
...
Controls Scheduler whether to support recurring event, refer to [this feature request](https://github.com/StephenChou1017/casta-scheduler/issues/8), default `true`.
...
### 4.SchedulerData.behaviors(See the [behaviors.js](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/behaviors.js) for details.)
...
DnDSource array that registered to Scheduler. Use [DnDSource](https://github.com/StephenChou1017/casta-scheduler/blob/master/src/DnDSource.js),
we can simplify the drag and drop coding in casta-Scheduler. Refer
